...
Auth            NTLM
" >> $CNTLM_CONFIG_FILE
echo `cntlm -v -c /etc/cntlm.conf -M "http://www.google.com" <<<$password | grep PassNT` >> $CNTLM_CONFIG_FILE
echo `cntlm -v -c /etc/cntlm.conf -M "http://www.google.com" <<<$password | grep PassLM` >> $CNTLM_CONFIG_FILE
echo "
" >> $CNTLM_CONFIG_FILE
